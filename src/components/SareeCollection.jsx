...
export default function SareeCollection({ currentLang, onOpenBooking }) {
  ...
  const filteredCatalog =
    selectedCategory === "all"
      ? sareeTypesCatalog
      : sareeTypesCatalog.filter(
          (item) =>
            item.category === selectedCategory || item.id === selectedCategory,
        );
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-3.5 py-1 rounded-full text-xs font-bold text-amber-300">
            ...
            <span>
              {t.typesPage?.headerTitle || "Complete Saree & Zari Catalog"}
            </span>
          </div>
          ...
          <p className="text-amber-100/70 text-sm sm:text-base">
            {t.types?.subtitle ||
              "We buy all varieties of old, second-hand, wedding, soft silk, and damaged pattu sarees at direct silk mill rates with doorstep spot cash."}
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-4">
            ...
            >
              {t.typesPage?.categories?.all || "All Types"} (
              {sareeTypesCatalog.length})
            </button>
            <button
            ...
            >
              {t.typesPage?.categories?.kanchipuram ||
            ...
            >
              {t.typesPage?.categories?.other_silk ||
                "Banarasi, Mysore, Tissue & Gadwal"}
            </button>
            <button
          ...
          {filteredCatalog.map((item) => {
            ...
            return (
              <div
                ...
                className="glass-panel p-5 gold-border-glow flex flex-col justify-between group transition-transform duration-500 hover:-translate-y-2 relative cursor-pointer"
              >
                <div>
            ...
            <div className="bg-neutral-950 border border-amber-500/40 rounded-2xl max-w-xl w-full p-6 text-left relative shadow-2xl">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="w-full btn-gold text-sm justify-center py-2.5"
                >
                  Book Pickup Now
                </button>
                <a
                  href="tel:6374067251"
                  className="w-full btn-crimson text-sm justify-center py-2.5"
                >
                  Call 63740 67251
                </a>
              </div>
