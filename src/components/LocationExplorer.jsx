...
export default function LocationExplorer({ currentLang, onOpenBooking }) {
  ...
  const currentState =
    statesData.find((s) => s.id === activeStateId) || statesData[0];

  const filteredCities = currentState.featuredCities.filter(
    (city) =>
  ...
  const sectionStyle = {
    padding: "64px 0",
    background:
      "linear-gradient(180deg, #0c0509 0%, rgba(115,0,25,0.12) 50%, #0c0509 100%)",
    ...
    <section id="locations" style={sectionStyle}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div
          ...
          style={{ textAlign: "center", maxWidth: 768, margin: "0 auto 40px" }}
        >
          <div
            style={{
              display: "inline-flex",
          ...
          <h2
            className="gold-text font-heading-luxury"
            style={{ fontSize: "2.25rem", fontWeight: 800, marginTop: 12 }}
          >
          ...
          >
            {t.locationsSection?.subtitle ||
              "We serve all major cities. Click any state to explore covered areas."}
          </p>
        </div>
              ...
              style={{
                ...
                borderColor:
                  activeStateId === state.id
                    ...
                    : "rgba(229,193,88,0.3)",
                transform:
                  activeStateId === state.id ? "scale(1.05)" : "scale(1)",
                boxShadow:
                  activeStateId === state.id
        ...
        {/* Search Bar */}
        <div
          style={{ maxWidth: 576, margin: "0 auto 40px", position: "relative" }}
        >
          <Search
            style={{
              width: 20,
          ...
          <input
            ...
            placeholder={
              t.locationsSection?.searchPlaceholder ||
              "Search city, pincode, or area..."
            }
          ...
          {filteredCities.length > 0 ? (
            filteredCities.map((city, idx) => (
              <div
                key={idx}
                onClick={() =>
                  navigate(`/city/${currentState.id}/${city.slug}`)
                }
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <span>
                        Doorstep Arrival: <strong>{city.time}</strong>
                      </span>
                    </div>
                    <div
                      style={{
            ...
            >
              No matching city for "{searchQuery}". We cover ALL towns. Call{" "}
              <strong>6374067251</strong>.
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            {currentState.seoDescription ||
              `Our mobile saree appraisal vehicles operate daily across all districts in ${currentState.name}. Sri Pattu & Zari Hub guarantees Kanchipuram mill cash payouts with zero traveling stress.`}
          </p>
        </div>
      </div>
    </section>
