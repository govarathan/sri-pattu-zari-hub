    ...
    <div style={{ minHeight: '100vh', padding: '4rem 1rem', maxWidth: '1200px', margin: '0 auto', color: '#f6f2ea' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        ...
        <p className="font-serif-luxury" style={{ fontSize: '1.2rem', color: '#e2e2e2', maxWidth: '700px', margin: '0 auto' }}>
          {t.typesPage?.headerSubtitle || 'From vintage Kanchipuram to damaged silk, we offer the best market price based on silver and gold zari content.'}
        </p>
      </div>
          ...
          <div key={saree.id} className="luxury-card glass-panel" style={{ borderRadius: '16px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <h2 className="font-heading-luxury" style={{ fontSize: '1.4rem', color: '#e5c158', margin: 0 }}>
                  {t.types[saree.titleKey] || t[saree.titleKey] || saree.titleKey}
                </h2>
              ...
              <p className="font-serif-luxury" style={{ color: '#e2e2e2', marginBottom: '1.25rem', lineHeight: '1.6', fontSize: '0.9rem' }}>
                {t.types[saree.descKey] || t[saree.descKey] || saree.descKey}
              </p>

              <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(229,193,88,0.2)', padding: '0.85rem', borderRadius: '8px', marginBottom: '1.25rem' }}>
                <strong style={{ color: '#e5c158', display: 'block', marginBottom: '0.25rem', fontSize: '0.85rem' }}>{t.typesPage?.puritySpecLabel || 'Purity Specification:'}</strong>
                <span style={{ fontSize: '0.85rem', color: '#f7e7a9' }}>{saree.purityInfo}</span>
              </div>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 1.5rem 0', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {saree.features.map((feature, idx) => (
                  <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem' }}>
                    <CheckCircle2 size={16} color="#34d399" style={{ marginTop: '0.1rem', flexShrink: 0 }} />
                    <span style={{ fontSize: '0.85rem', lineHeight: '1.4', color: '#e2e2e2' }}>{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <button onClick={onOpenBooking} className="btn-gold-luxury" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', fontWeight: 'bold', fontSize: '0.95rem' }}>
                  <ShieldCheck size={18} />
                  {t.typesPage?.sellBtn || 'Sell This Saree at Doorstep'}
                </button>
                <a href={`https://wa.me/916374067251?text=Hi, I want to sell my ${saree.id} saree`} target="_blank" rel="noopener noreferrer" className="btn-emerald" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', textDecoration: 'none', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', backgroundColor: '#0b5e56', color: '#f6f2ea', fontSize: '0.95rem' }}>
      ...
      <div className="luxury-card" style={{ padding: '3rem 2rem', borderRadius: '16px', textAlign: 'center', backgroundImage: 'linear-gradient(45deg, #0c0509 0%, #1a0f14 100%)', border: '1px solid #b88d22' }}>
        <h2 className="font-heading-luxury" style={{ fontSize: '2rem', marginBottom: '1.5rem', color: '#e5c158' }}>{t.typesPage?.bulkTitle || 'Have multiple silk sarees to sell?'}</h2>
        <p className="font-serif-luxury" style={{ fontSize: '1.1rem', marginBottom: '2rem', maxWidth: '600px', margin: '0 auto 2rem auto' }}>
          {t.typesPage?.bulkDesc || 'We buy bulk family collections and vintage heirloom silk sarees with instant spot cash at your doorstep.'}
        </p>
