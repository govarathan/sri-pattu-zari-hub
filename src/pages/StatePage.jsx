...
import { MapPin, Phone, Clock, ArrowRight, ChevronRight, MessageSquare, Navigation, ShieldCheck } from 'lucide-react';
    ...
    <div style={{ minHeight: '100vh', padding: '2rem 1rem', maxWidth: '1200px', margin: '0 auto', color: '#f6f2ea' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '2rem', marginBottom: '4rem' }}>
        {state.featuredCities && state.featuredCities.map((city, index) => {
          const citySlug = city.slug || city.name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
          return (
            <div key={index} className="luxury-card glass-panel gold-border-glow" style={{ padding: '2rem', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                {city.time && (
                  <div className="gold-badge" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.25rem 0.5rem', borderRadius: '20px', fontSize: '0.8rem', backgroundColor: 'rgba(229,193,88,0.1)' }}>
                    <Clock size={12} />
                    <span>{city.time}</span>
                  </div>
                )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.95rem' }}>
                {city.highlight && (
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                    <MapPin size={16} color="#e5c158" style={{ marginTop: '0.2rem', flexShrink: 0 }} />
                    <span style={{ lineHeight: '1.4' }}>{city.highlight}</span>
                  </div>
                )}
                {city.pincodes && (
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                    <Navigation size={16} color="#e5c158" style={{ marginTop: '0.2rem', flexShrink: 0 }} />
      ...
      <div className="glass-panel" style={{ padding: '2rem', borderRadius: '12px', marginBottom: '4rem', borderLeft: '4px solid #e5c158', backgroundColor: 'rgba(255,255,255,0.03)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
          {state.seoKeywords && state.seoKeywords.split(', ').map((keyword, i) => (
            <span key={i} style={{ fontSize: '0.8rem', padding: '0.25rem 0.75rem', borderRadius: '20px', backgroundColor: 'rgba(229,193,88,0.1)', color: '#b88d22', border: '1px solid rgba(229,193,88,0.3)' }}>
              {keyword}
            </span>
          ))}
        </div>
